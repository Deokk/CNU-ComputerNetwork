...
def make_header(Data, size, sequence):
    ...
    data = sequence + Src_Address + Dst_Address + Zeros + Protocol + UDP_Length + Src_Port + Dst_Port + Length + init_checksum + Data
    # 생성한 data 를 바탕으로 정의한 함수를 이용해 checksum 을 계산해줍니다.
    checksum = calc_checksum(data)
    # 계산한 checksum 을 출력합니다.
    print('checksum:', checksum)
    # 계산한 checksum 으로 빈 checksum 의 자리를 채워줍니다.
    data = sequence + Src_Address + Dst_Address + Zeros + Protocol + UDP_Length + Src_Port + Dst_Port + Length + checksum + Data
    # 생성한 data 를 반환합니다.
    return data

# 수신한 ack 를 이용해 frame 을 설정하는 함수입니다.
# ack 가 '0'이나 '1'이 아닐 때 sys.exit()을 사용합니다.
def set_frame(rcv_ack):
    if rcv_ack == '0':
        return '0'
    elif rcv_ack == '1':
        return '1'
    else:
        print("Can't receive ack")
        print("sys exit")
        sys.exit()
...
def sender_send(f_name, addr):
    ...
    if os.path.isfile(f_name):
        ...
        while check != 0:
            # header 의 길이 41을 1024에서 뺀 만큼 file 을 읽어줍니다.
            chunk_file = read_file.read(1024 - 41)
            # 읽어준 chunk_file 을 utf-8 로 decode 한 다음
            # make_header 함수를 이용해 header 를 만들어줍니다.
            data = make_header(chunk_file.decode('utf-8'), len(chunk_file), frame)
            # 만들어 준 데이터를 socket 을 이용해 송신합니다.
            # 이때 utf-8 로 인코딩하여 송신해줍니다.
            server_socket.sendto(data.encode('utf-8'), addr)
            print('sending index :', frame)
            while 1:
                try:
                    # receiver 로부터 Ack 를 수신합니다.
                    chunk_file, temp = server_socket.recvfrom(1024)
                    break
            ...
            ack = chunk_file.decode('utf-8')[:1]
            print('received ack index :', ack)
            frame = set_frame(ack)
